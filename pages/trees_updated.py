        ...
        with split.before:
            with ui.element('div').classes('w-full h-full').style('width:100%; height:100%;'):
                with ui.card().props('flat').classes('w-full h-full m-0 p-4'):
                    ...
                    # HTML: hidden inputs + controls (Choose File / Use Camera / Clear) + filename label + preview
                    ui.html("""
                    <div id="ct-uploader" style="display:inline-block;">
                    <!-- Hidden file inputs -->
                    <input id="ct-file" type="file" accept="image/*" style="display:none">
                    <input id="ct-file-camera" type="file" accept="image/*" capture="environment" style="display:none">

                    <!-- Controls -->
                    <div style="display:flex; gap:10px; align-items:center; margin-bottom:8px;">
                        <label for="ct-file" class="ct-btn">Choose File</label>
                        <label for="ct-file-camera" class="ct-btn">Use Camera</label>

                        <!-- File name / status -->
                        <span id="ct-filename" class="ct-filename">No file selected</span>

                        <!-- Clear selection -->
                        <button id="ct-clear" type="button" class="ct-btn ct-btn-danger" disabled>Clear</button>
                    </div>

                    <!-- Live thumbnail preview -->
                    <img id="ct-preview" src=""
                        style="display:none; width:96px; height:96px; object-fit:cover; border-radius:6px; border:1px solid #ddd;" />
                    </div>
                    """)

                    # Minimal styles for the buttons and filename label
                    ui.add_head_html("""
                    <style>
                    .ct-btn {
                    ...
                    .ct-btn-danger { background:#fff; border-color:#e0e0e0; }
                    .ct-btn-danger:hover { background:#f8f8f8; }
                    .ct-filename {
                    font-size: 0.9rem; color:#333; min-width: 140px;
                    ...
                    # JS: state, wiring, and helpers (works on desktop & mobile)
                    ui.add_body_html("""
                    <script>
                    (function () {
                    const $ = (id) => document.getElementById(id);

                    // ----- State + helpers with logs -----
                    function log(...args){ try{ console.log('[ctUpload]', ...args); }catch(e){} }

                    function setState(file) {
                        const preview = $('ct-preview');
                        const clearBtn = $('ct-clear');
                        const nameLbl = $('ct-filename');
                        if (!preview || !clearBtn || !nameLbl) {
                        log('setState: missing DOM parts; preview?', !!preview, 'clear?', !!clearBtn, 'label?', !!nameLbl);
                        return;
                        }
                        if (file) {
                        ...
                        log('setState: file set ->', name);
                        } else {
                        nameLbl.textContent = 'No file selected';
                        preview.src = '';
                        preview.style.display = 'none';
                        clearBtn.disabled = true;
                        log('setState: cleared');
                        }
                    }

                    // Expose a stable upload/clear API for submit()
                    window.ctUpload = {
                        clear() {
                        const fileInput = $('ct-file');
                        const camInput = $('ct-file-camera');
                        if (fileInput) fileInput.value = '';
                        if (camInput)  camInput.value = '';
                        setState(null);
                        log('clear() done');
                        },
                        async upload(apiUrl, treeId) {
                        const fileInput = $('ct-file');
                        const camInput = $('ct-file-camera');
                        const f = (camInput?.files?.[0]) || (fileInput?.files?.[0]) || null;
                        log('upload(): chosen from', camInput?.files?.length ? 'camera' : (fileInput?.files?.length ? 'file' : 'none'));
                        if (!f) return 'no_file';
                        const fd = new FormData();
                        fd.append('file', f, f.name || 'upload.jpg');
                        const url = `${apiUrl}/trees/${treeId}/photo`;
                        log('upload(): POST', url, 'name=', f.name, 'size=', f.size);
                        const res = await fetch(url, { method: 'POST', body: fd });
                        if (!res.ok) {
                            let msg=''; try { msg = await res.text(); } catch(e) {}
                            log('upload(): server error', res.status, msg);
                    ...
                    function wireOnce() {
                        ...
                        if (!fileInput || !camInput || !clearBtn || !nameLbl || !preview) {
                        log('wireOnce: missing elements; will retry…');
                        return false;
                        }

                        // Avoid multiple bindings: remove previous to be safe
                        fileInput.onchange = null;
                        camInput.onchange  = null;
                        clearBtn.onclick   = null;

                        fileInput.addEventListener('change', (ev) => {
                        // clear the camera input to avoid ambiguity
                        if (camInput) camInput.value = '';
                        const f = ev.target.files?.[0] || null;
                        log('fileInput change: hasFile?', !!f);
                        setState(f);
                        });

                        camInput.addEventListener('change', (ev) => {
                        if (fileInput) fileInput.value = '';
                        const f = ev.target.files?.[0] || null;
                        log('cameraInput change: hasFile?', !!f);
                        setState(f);
                        });

                        clearBtn.addEventListener('click', () => {
                        log('clear button clicked');
                        window.ctUpload.clear();
                        });

                        // Initialize label
                        setState(null);
                        log('wireOnce: listeners attached');
                        return true;
                    }

                    // Try to wire now…
                    if (!wireOnce()) {
                        // …and keep trying a few times (elements may mount a bit later)
                        let attempts = 0;
                        const timer = setInterval(() => {
                        attempts += 1;
                        if (wireOnce() || attempts > 20) {
                            clearInterval(timer);
                            log('wiring stopped. success?', attempts <= 20);
                        }
                        }, 200);
                    }

                    // Also observe DOM changes under the uploader container to re-wire if inputs get replaced
                    const container = document.getElementById('ct-uploader');
                    if (container && 'MutationObserver' in window) {
                        const mo = new MutationObserver(() => {
                        // If the inputs were replaced, re-wire
                        const ok =
                            document.getElementById('ct-file') &&
                            document.getElementById('ct-file-camera') &&
                            document.getElementById('ct-clear') &&
                            document.getElementById('ct-filename') &&
                            document.getElementById('ct-preview');
                        if (ok) {
                            // Re-attach (idempotent due to handler reset)
                            wireOnce();
                        }
                        });
                        ...
                        log('MutationObserver not available or container missing');
                    }
                    })();
                    </script>
                    """)
                    ...
                    async def submit(do_print: bool):
                        ...
                        try:
                            ...
                            # 2) upload photo (if any) via browser fetch; await returned string
                            try:
                                with client:
                                    js_result = await ui.run_javascript(f"""
                                        (async () => {{
                                            const inp = document.getElementById('ct-file');
                                            const cam = document.getElementById('ct-file-camera');
                                            const f = (cam && cam.files && cam.files[0]) || (inp && inp.files && inp.files[0]) || null;
                                            if (!f) return 'no_file';
                                            const fd = new FormData();
                                            fd.append('file', f, f.name || 'upload.jpg');
                                            const res = await fetch('{API_URL}/trees/{int(data["id"])}/photo', {{
                                                method: 'POST',
                                                body: fd
                                            }});
                                            if (!res.ok) {{
                                ...
                                elif js_result == 'no_file':
                                    print('No photo selected.')
                                else:
                                    notify(f'Photo upload failed: {js_result}', 'warning')
                            except Exception as ex_up:
                                notify(f'Photo upload error: {ex_up}', 'warning')

                            # 3) optionally open the label PDF in a new tab
                            if do_print:
                                try:
                            ...
                            # clear the native file input and preview (handled by our JS helper)
                            try:
                                with client:
                                    await ui.run_javascript("if (window.ctUpload) window.ctUpload.clear();")
                            except Exception:
                                pass

                            # refresh computed fields and tables
                            refresh_est()
                            await fill_next_no()
